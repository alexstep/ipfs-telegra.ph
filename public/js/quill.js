...
/***/ function(module, exports, __webpack_require__) {
	...
	var Quill = function () {
      ...
      this.fileSizeLimit = this.options.fileSizeLimit;
      this.fileSizeLimitCallback = this.options.fileSizeLimitCallback;
      this.updatePhoto = this.options.updatePhoto || function(file, callback) { callback && callback(file);
      };
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Selection = function () {
	  ...
	  _createClass(Selection, [{
        ...
        var rootOffsetTop = offsetTop(this.root);
        var rangeTop = bounds.top + rootOffsetTop - document.body.scrollTop;
        var rangeBottom = bounds.bottom + rootOffsetTop - document.body.scrollTop;
        if (window.offsetHeight < rangeBottom) {
          ...
          document.body.scrollTop = line.domNode.offsetTop + line.domNode.offsetHeight + rootOffsetTop - window.offsetHeight;
        } else if (rangeTop < 0) {
          ...
          document.body.scrollTop = _line.domNode.offsetTop + rootOffsetTop;
        }
	    }
	  }, {
	    key: 'setNativeRange',
...
/***/ function(module, exports, __webpack_require__) {
	  ...
	  _createClass(Clipboard, [{
	    ...
	    value: function onPaste(e) {
	      ...
	      var doPaste = function(paste) {
	        ...
	        _this3.quill.emitter.emit(_this3.quill.emitter.constructor.events.TEXT_PASTE, delta, _quill2.default.sources.USER);
	      }
	      if (typeof e.clipboardData !== 'undefined' &&
	          (copiedData = e.clipboardData.items)) {
	        for (var i = 0; i < copiedData.length; i++) {
	          if ((copiedData[i].kind == 'file') &&
	              (copiedData[i].type.indexOf('image/') == 0)) {
	            var imageFile = copiedData[i].getAsFile();
              _this3.quill.updatePhoto(imageFile, function (imageFile) {
	              if (_this3.quill.fileSizeLimit && imageFile.size > _this3.quill.fileSizeLimit) {
	                _this3.quill.fileSizeLimitCallback && _this3.quill.fileSizeLimitCallback();
	                return;
	              }
		            ...
		            reader.onload = function (e) {
		              ...
		              doPaste(_this3.convert(img.outerHTML));
		            };
	      ...
	      if (!isFile) {
	        this.container.style.top = rangeBounds.top + 'px';
	        this.container.focus();
	        setTimeout(function () {
	          doPaste(_this3.convert());
	        }, 1);
	      }
	    }
	  }]);
	...
	function matchSpacing(node, delta) {
	  if (isLine(node) && node.nextElementSibling != null && !deltaEndsWith(delta, '\n\n')) {
	    ...
	    var nodeHeight = node.offsetHeight + marginTop + marginBottom;
	          ...
	          if (fileInput == null) {
	            ...
	            fileInput.addEventListener('change', function () {
                  ...
                  var file = fileInput.files[0];
		              _this3.quill.updatePhoto(file, function (file) {
	                  if (_this3.quill.fileSizeLimit && file.size > _this3.quill.fileSizeLimit) {
	                    _this3.quill.fileSizeLimitCallback && _this3.quill.fileSizeLimitCallback();
	                    return;
	                  }
		                var reader = new FileReader();
